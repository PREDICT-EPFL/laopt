...
#ifndef LAOPT_BLOCK_SPARSE_MATRIX
#define LAOPT_BLOCK_SPARSE_MATRIX
 ...
 * \tparam BlockAtCompileTime_ The block layout option. It takes the following values
...
class BlockSparseMatrix : public SparseMatrixBase<BlockSparseMatrix<Scalar_, BlockAtCompileTime_, Options_, StorageIndex_>> {
  typedef SparseMatrixBase<BlockSparseMatrix<Scalar_, BlockAtCompileTime_, Options_, StorageIndex_>> Base;

  template<typename, int, int, typename>
  friend class BlockSparseMatrix;

public:
  EIGEN_SPARSE_PUBLIC_INTERFACE(BlockSparseMatrix);
 ...
 public:
  ...
  BlockSparseMatrix()
      : m_innerBSize(0),
        ...
        m_nonzerosblocks(0),
        m_nonzeros(0),
        m_allocatedNonzeros(0),
        m_values(0),
        m_zeroValues(0),
        m_zeroValuesSize(0),
        m_allocatedZeroValuesSize(0),
        m_blockPtr(0),
        m_indices(0),
        m_outerIndex(0),
  ...
  BlockSparseMatrix(const BlockSparseMatrix& other)
      : m_innerBSize(other.m_innerBSize),
        ...
        m_allocatedNonzeros(other.m_allocatedNonzeros),
        m_zeroValuesSize(other.m_zeroValuesSize),
        m_allocatedZeroValuesSize(other.m_allocatedZeroValuesSize),
        m_blockPtr(0),
    ...
    if (m_blockSize == Dynamic) {
      m_innerOffset = internal::conditional_aligned_new_auto<StorageIndex, true>(m_innerBSize + 1);
      m_outerOffset = internal::conditional_aligned_new_auto<StorageIndex, true>(m_outerBSize + 1);
      m_blockPtr = internal::conditional_aligned_new_auto<StorageIndex, true>(m_nonzerosblocks + 1);

      internal::smart_copy(other.m_innerOffset, other.m_innerOffset + m_innerBSize + 1, m_innerOffset);
      internal::smart_copy(other.m_outerOffset, other.m_outerOffset + m_outerBSize + 1, m_outerOffset);
      internal::smart_copy(other.m_blockPtr, other.m_blockPtr + m_nonzerosblocks + 1, m_blockPtr);
    }

    m_values = internal::conditional_aligned_new_auto<Scalar, true>(m_allocatedNonzeros);
    m_zeroValues = internal::conditional_aligned_new_auto<Scalar, true>(m_zeroValuesSize);
    m_indices = internal::conditional_aligned_new_auto<StorageIndex, true>(m_nonzerosblocks);
    m_outerIndex = internal::conditional_aligned_new_auto<StorageIndex, true>(m_outerBSize + 1);

    internal::smart_copy(other.m_values, other.m_values + m_allocatedNonzeros, m_values);
    internal::smart_copy(other.m_zeroValues, other.m_zeroValues + m_allocatedZeroValuesSize, m_zeroValues);
    internal::smart_copy(other.m_indices, other.m_indices + m_nonzerosblocks, m_indices);
    internal::smart_copy(other.m_outerIndex, other.m_outerIndex + m_outerBSize + 1, m_outerIndex);
  }
  ...
  friend void swap(BlockSparseMatrix& first, BlockSparseMatrix& second) {
    ...
    std::swap(first.m_allocatedNonzeros, second.m_allocatedNonzeros);
    std::swap(first.m_values, second.m_values);
    std::swap(first.m_zeroValues, second.m_zeroValues);
    std::swap(first.m_zeroValuesSize, second.m_zeroValuesSize);
    std::swap(first.m_allocatedZeroValuesSize, second.m_allocatedZeroValuesSize);
  ...
  // Destructor
  ~BlockSparseMatrix() {
    internal::conditional_aligned_delete_auto<StorageIndex, true>(m_outerIndex, m_outerBSize + 1);
    if (m_blockSize == Dynamic) {
        internal::conditional_aligned_delete_auto<StorageIndex, true>(m_innerOffset, m_innerBSize + 1);
        internal::conditional_aligned_delete_auto<StorageIndex, true>(m_outerOffset, m_outerBSize + 1);
        internal::conditional_aligned_delete_auto<StorageIndex, true>(m_blockPtr, m_nonzerosblocks + 1);
    }
    internal::conditional_aligned_delete_auto<StorageIndex, true>(m_indices, m_nonzerosblocks);
    internal::conditional_aligned_delete_auto<Scalar, true>(m_values, m_allocatedNonzeros);
    internal::conditional_aligned_delete_auto<Scalar, true>(m_zeroValues, m_allocatedZeroValuesSize);
  }
  ...
  inline BlockSparseMatrix(const SparseMatrixBase<Derived>& spmat) : BlockSparseMatrix() {
    EIGEN_STATIC_ASSERT((m_blockSize != Dynamic), THIS_METHOD_IS_ONLY_FOR_FIXED_SIZE);
    eigen_assert(spmat.rows() % m_blockSize == 0 && "Rows must be a multiple of blockSize");
    eigen_assert(spmat.cols() % m_blockSize == 0 && "Cols must be a multiple of blockSize");

    resize(spmat.rows() / m_blockSize, spmat.cols() / m_blockSize);
    *this = spmat;
  }
  ...
  inline BlockSparseMatrix& operator=(const SparseMatrixBase<Derived>& spmat) {
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      for (StorageIndex j = blockOuterIndex(bj); j < blockOuterIndex(bj + 1); ++j) {
        typename EvaluatorType::InnerIterator it_spmat(evaluator, j);
      ...
      for (StorageIndex j = blockOuterIndex(bj); j < blockOuterIndex(bj + 1); ++j) {
        ...
        for (; it_spmat; ++it_spmat) {
          ...
          if (m_blockSize == Dynamic) {
            ...
            idxVal += (j - blockOuterIndex(bj)) * blockInnerSize(bi) + it_spmat.index() - m_innerOffset[bi];
          } else {
            // All blocks before
  ...
  void setBlockStructure(const MatrixType& blockPattern) {
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      for (StorageIndex idx = 0; idx < StorageIndex(nzBlockIdx.size()); ++idx) {
        StorageIndex offset = m_outerIndex[bj] + idx;  // offset in m_indices
        m_indices[offset] = nzBlockIdx[idx];
        if (m_blockSize == Dynamic) {
            m_blockPtr[offset + 1] = m_blockPtr[offset] + blockInnerSize(nzBlockIdx[idx]) * blockOuterSize(bj);
        }
  ...
  inline void resize(Index brow, Index bcol) {
    const Index innerBSize = IsColMajor ? brow : bcol;
    const Index outerBSize = IsColMajor ? bcol : brow;

    if ((m_outerIndex == 0) || (m_outerOffset == 0) || (m_outerBSize != outerBSize)) {
        m_outerIndex = internal::conditional_aligned_realloc_new_auto<StorageIndex, true>(m_outerIndex, outerBSize + 1, m_outerBSize + 1);
        if (m_blockSize == Dynamic) {
            m_outerOffset = internal::conditional_aligned_realloc_new_auto<StorageIndex, true>(m_outerOffset, outerBSize + 1, m_outerBSize + 1);
        }
        m_outerBSize = outerBSize;
    }

    if ((m_innerOffset == 0) || (m_innerBSize != innerBSize)) {
        if (m_blockSize == Dynamic) {
            m_innerOffset = internal::conditional_aligned_realloc_new_auto<StorageIndex, true>(m_innerOffset, innerBSize + 1, m_innerBSize + 1);
        }
        m_innerBSize = innerBSize;
    }

    std::fill_n(m_outerIndex, m_outerBSize + 1, StorageIndex(0));
  }
  ...
  inline void setBlockLayout(const VectorXi& rowBlocks, const VectorXi& colBlocks) {
    ...
    eigen_assert(m_blockSize == Dynamic && "MATRIX NEEDS TO BE VARIABLE SIZE");
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
        for (StorageIndex bi = 0; bi < m_innerBSize; ++bi) {
            Index blockSize = outerBlocks[bj] * innerBlocks[bi];
            m_nonzeros += blockSize;
            m_zeroValuesSize = std::max(m_zeroValuesSize, blockSize);
        }
    }
  }
  ...
  inline void setBlockLayout(const BlockSparseMatrix<OtherScalar, OtherBlockAtCompileTime, Options, StorageIndex>& other) {
      ...
      for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
          for (StorageIndex bi = 0; bi < m_innerBSize; ++bi) {
              Index blockSize = blockOuterSize(bj) * blockInnerSize(bi);
              m_nonzeros += blockSize;
              m_zeroValuesSize = std::max(m_zeroValuesSize, blockSize);
          }
      }
  ...
  inline void reserve(const Index nonzerosblocks) {
    ...
    if (m_blockSize != Dynamic) {
        ...
        m_zeroValuesSize = m_blockSize * m_blockSize;
        m_blockPtr = 0;
    } else {
        // m_nonzeros is already computed in setBlockLayout()
        // m_zeroValuesSize is already computed in setBlockLayout()
        m_blockPtr = internal::conditional_aligned_realloc_new_auto<StorageIndex, true>(m_blockPtr, nonzerosblocks + 1, m_nonzerosblocks + 1);
    }

    m_indices = internal::conditional_aligned_realloc_new_auto<StorageIndex, true>(m_indices, nonzerosblocks, m_nonzerosblocks);
    m_values = internal::conditional_aligned_realloc_new_auto<Scalar, true>(m_values, m_nonzeros, m_allocatedNonzeros);
    m_zeroValues = internal::conditional_aligned_realloc_new_auto<Scalar, true>(m_zeroValues, m_zeroValuesSize, m_allocatedZeroValuesSize);
    std::fill_n(m_zeroValues, m_zeroValuesSize, Scalar(0));

    m_nonzerosblocks = nonzerosblocks;
    m_allocatedNonzeros = m_nonzeros;
    m_allocatedZeroValuesSize = m_zeroValuesSize;
  }
  ...
  Map<const BlockScalar> coeff(Index brow, Index bcol) const {
    ...
    if (m_indices[offset] == inner) {
      ...
      return Map<const BlockScalar>(m_zeroValues, rsize, csize);
  }
...
//  }
 ...
 protected:
  ...
  Index m_allocatedNonzeros;      // Allocated nonzeros
  Scalar* m_values;                // Values stored block column after block column (size m_nonzeros)
  Scalar* m_zeroValues;            // Buffer filled with zeros for return of coeff() (size m_zeroValuesSize)
  Index m_zeroValuesSize;          // Size of the zero filled buffer
  Index m_allocatedZeroValuesSize; // Allocated size of the zero filled buffer
...
template <typename Scalar_, int BlockAtCompileTime_, int Options_, typename StorageIndex_>
class BlockSparseMatrix<Scalar_, BlockAtCompileTime_, Options_, StorageIndex_>::BlockInnerIterator {
 public:
  ...
  inline Map<BlockScalar> valueRef() {
    return Map<BlockScalar>(&(m_mat.m_values[m_mat.blockPtr(m_id)]), blockRows(), blockCols());
  }
  // Block inner index
 ...
 protected:
  const BlockSparseMatrix<Scalar_, BlockAtCompileTime_, Options_, StorageIndex>& m_mat;
  const Index m_outer;
  Index m_id;
  Index m_end;
};
...
template <typename Scalar_, int BlockAtCompileTime_, int Options_, typename StorageIndex_>
class BlockSparseMatrix<Scalar_, BlockAtCompileTime_, Options_, StorageIndex_>::InnerIterator {
 public:
  ...
  inline Scalar& valueRef() { return itb.valueRef().coeffRef(m_id - m_start, m_offset); }
...
template <typename Scalar_, int BlockAtCompileTime_, int Options_, typename StorageIndex_>
struct evaluator<BlockSparseMatrix<Scalar_, BlockAtCompileTime_, Options_, StorageIndex_>>
        : evaluator_base<BlockSparseMatrix<Scalar_, BlockAtCompileTime_, Options_, StorageIndex_>> {
...
#endif  // LAOPT_BLOCK_SPARSE_MATRIX

...
struct Block
{
	...
	void setOffsets(const Eigen::SparseMatrix<Scalar>& S)
	{
	...
	: Eigen::SparseMatrix<Scalar>
{
	...
	BSMatrix()
	{
		this->resize(RowsAtCompileTime, ColsAtCompileTime);
	}
...
private:
	 ...
	 */
	template<typename var>
	int setColumnSparsity(int column, int& offset)
	{
		int nnz_block = 0;
		int nnz = 0;
		auto innerIndexPtr = this->innerIndexPtr();
        auto l = { (
        	nnz_block =
...
public:
	...
	void finalize()
	{
		...
		this->reserve(BS::accumulate(m_constraints, 0, 
        ...
        auto l = { (
        	column += [&column, &offset, this](){
				int nnz = 0;
				for(int i=0; i<Variables::size; i++)
				{
					nnz = setColumnSparsity<Variables>(i, offset);
					this->outerIndexPtr()[column + i + 1] = 
						this->outerIndexPtr()[column + i] + nnz;
				}
		...
		this->applyBlockwise([&](auto& blk){blk.setOffsets(*this);});
	}
};

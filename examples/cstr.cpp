...
class cstr_ocp : public ContinuousOCP<cstr_ocp, Approximation, DENSE>
{
public:
    ~cstr_ocp() = default;
    cstr_ocp()
...
int main(void)
{
    ...
    using Solver = MySolver<cstr_ocp, admm>;
    Solver::nlp_eq_constraints_t constraint;
    Solver::nlp_eq_jacobian_t jacobian;

    jacobian.resize(Solver::NUM_EQ, Solver::VAR_SIZE);

    solver.problem.equalities_linearised(solver.primal_solution(), solver.parameters(), constraint, jacobian);
